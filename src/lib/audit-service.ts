...
const RULE_SCREENING_SYSTEM_PROMPT = '你是一名商品名称意图识别助手。请只根据商品名称判断是否需要执行指定规则，输出只能是“命中”或“不命中”。';
...
function buildRuleScreeningPrompt(rule: AuditRule, productName: string) {
  const customPrompt = rule.screeningPrompt?.trim();
  if (customPrompt) {
    return customPrompt.replace('{product_name}', productName.trim());
  }

  return `请仅根据商品名称判断，该商品是否可能触发“${rule.name}”这条审核规则。

规则说明：
${rule.prompt}

待判断商品名称：${productName.trim()}

如果这个商品名称从语义上看需要进入这条规则的后续页面审核，请只输出“命中”。
如果从商品名称看明显不属于这条规则的关注范围，请只输出“不命中”。

不要补充解释，不要返回 JSON，不要考虑飞书字段。`;
}

async function matchRulesByProductName(
  productName: string,
  rules: AuditRule[],
  modelConfig?: ModelApiConfig | null
) {
  const enabledRules = rules.filter((rule) => rule.enabled);
  if (enabledRules.length === 0) return { matchedRules: [] as AuditRule[], analysis: '未启用任何规则。' };

  const normalizedName = productName.trim();
  if (!normalizedName) {
    return { matchedRules: enabledRules, analysis: '商品名称为空，无法做名称意图筛选，默认放行到全部已启用规则。' };
  }

  const resolvedModel = resolveModel(modelConfig, '');
  if (!modelConfig?.apiKey?.trim() || !resolvedModel) {
    return { matchedRules: enabledRules, analysis: '当前未配置可用模型，无法执行规则级名称筛选，默认放行到全部已启用规则。' };
  }
  ...
  for (const rule of enabledRules) {
    const response = await callTextOnlyCompletion(
      modelConfig,
      resolvedModel,
      RULE_SCREENING_SYSTEM_PROMPT,
      buildRuleScreeningPrompt(rule, normalizedName)
    );
    const normalizedResponse = response.replace(/\s+/g, '').trim();
    const isMatched =
      normalizedResponse.includes('命中') && !normalizedResponse.includes('不命中');

    analysisLines.push(`【${rule.name}】${isMatched ? '命中' : '不命中'}：${normalizedResponse || '空响应'}`);
    if (isMatched) {
...
export async function screenAuditRulesByProductName(
  productName: string,
  rules?: AuditRules,
  modelConfig?: ModelApiConfig | null
) {
  const enabledRules = rules?.rules?.filter((rule) => rule.enabled) || [];
  return matchRulesByProductName(productName, enabledRules, modelConfig);
}
...
export async function runAudit(
  ...
  const matched = preScreening || (await matchRulesByProductName(productName, enabledRules, modelConfig));
  if (matched.matchedRules.length === 0) {
    return {
      result: {
        conclusion: '未审核',
        violations: [],
        analysis: `规则级商品名筛选未命中任何已启用规则，本条链接跳过页面抓取与审核。\n\n${matched.analysis}`,
      },
      ruleResults: [
        {
          ruleId: 'rule-name-screening',
          ruleName: '规则级商品名筛选',
          model: resolveModel(modelConfig, '') || 'system',
          conclusion: '未命中',
          violations: [],
  ...
  return {
    result: {
      ...aggregated,
      analysis: `规则级商品名筛选命中：${matched.matchedRules.map((rule) => rule.name).join('、')}\n\n${matched.analysis}\n\n${aggregated.analysis}`,
    },
    ruleResults: internalRuleResults.map((item) => ({
      ruleId: item.ruleId,
